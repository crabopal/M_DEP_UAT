...
Процедура ПриОткрытии(Отказ)
	...
	Объект.КомПорт 						= ПолучитьЗначениеКомПорт();	
	Объект.ТипСчитывателяКарт 			= "RFID";
	
	Если Объект.ТипСчитывателяКарт="RFID" Тогда
		
		Если ЗначениеЗаполнено(Объект.КомПорт)  Тогда //Если задан порт кодоса
			Если ПодключитьВнешнююКомпоненту("AddIn.kodos_rd") Тогда
				kodos_rd = Новый("AddIn.kodos_rd");
			ИначеЕсли ПодключитьВнешнююКомпоненту("C:\ZR1C\ZR1C.dll","Comp",ТипВнешнейКомпоненты.Native) Тогда
				ZR1C = Новый("AddIn.Comp.ZR1CExtension");
			Иначе	
				Сообщить("Не подключена внешняя библиотека kodos_rd.dll и ZR1C!");
			КонецЕсли;
			
			ОткрытьПорт(Объект.КомПорт);		
		КонецЕсли;
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	ЭтоRFID = (Объект.ТипСчитывателяКарт="RFID"  И Источник="kodos_rd" И Событие="Считан код") ИЛИ Событие="Считан код" ИЛИ Событие="ДанныеКарты" 
	ИЛИ
	(Объект.ТипСчитывателяКарт="RFID"  И  ВРЕГ(СтрЗаменить(Источник," ",""))=ВРЕГ(СтрЗаменить(Объект.КомПорт," ","")) И Событие="TracksData"); //для ZR1C
	ЭтоШтрихКод = 	 (Объект.ТипСчитывателяКарт="SCAN"  И Событие="Штрихкод");
	...
	Если (ЭтоRFID
		...
		//Сообщить("Считан код карты: "+Данные); 
	КонецЕсли;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если kodos_rd <> Неопределено Тогда	
		//Закрываем порт
		kodos_rd.ЗакрытьПорт(); 
	КонецЕсли;
	
	Если ZR1C<>Неопределено И ЗначениеЗаполнено(DevID) Тогда
		ZR1C.Отключить(DevID);
	КонецЕсли;
...
Функция СписокПЛВодителя(Водитель)
	...
	//++БИТ Лыткин Е.И.13.05.26 г. добавление отбора по дате выезда и дате возвращения. Переименовал параметр "НачалоСуток", добавил условия по дате возвращения. 
	"ВЫБРАТЬ
	...
	|	И уатПутевойЛист.Проведен
	|	И НЕ уатПутевойЛист.ЗакрытВодителем
	|	И (уатПутевойЛист.ДатаВыезда >= &НачалоСуток
	|			ИЛИ уатПутевойЛист.ДатаВыезда < &НачалоСуток
	|				И (уатПутевойЛист.ДатаВозвращения >= &НачалоСуток
	|					ИЛИ уатПутевойЛист.ДатаВозвращения = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)))");
	Запрос.УстановитьПараметр("Водитель", Водитель);
	Запрос.УстановитьПараметр("НачалоСуток", НачалоДня(ТекущаяДата()));
	//--БИТ Лыткин Е.И.13.05.26 г.добавление отбора по дате выезда и дате возвращения. Переименовал параметр "НачалоСуток", добавил условия по дате возвращения. 
	мсвПЛ = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");					  
			...
			Если ТипЗнч(ТекПЛ) = Тип("ДокументСсылка.уатПутевойЛист") Тогда
				Если РазрешитьАвторизациюПоШтрихкоду Тогда
					Если ЗначениеЗаполнено(ТекПЛ) И ПЛдоступенДляЗакрытияВодителем(ТекПЛ) Тогда
						...
						//+! Гаспирович ВВ БИТ
						уатОбщегоНазначенияКлиент.УстановитьОтборУСписка(СписокДокументов.Отбор, "ДатаВыезда", НачалоДня(ТекущаяДата()),ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
	...
	Если НЕ СравнитьТабличныеЧасти("Задание") Тогда
		...
		Для Каждого ТекСтрока Из ПутевойЛист_Задание Цикл
			...
			Если флСтрокаНайдена Тогда
				...
				ТекСтрокаПоиск.Подразделение = ТекСтрока.Подразделение;
				//+! Бит! 04.03.26 Гаспирович В.В.
				//НоваяСтрока.КАУ = ТекСтрока.КАУ;
				//-! Бит! Гаспирович В.В. 
				ТекСтрокаПоиск.КАУ = ТекСтрока.КАУ;
			Иначе
...
&НаКлиенте
Процедура ОткрытьПорт(ИмяПорта)
	Если kodos_rd<>Неопределено Тогда
		Попытка
		...
		КонецПопытки;	
	ИначеЕсли ZR1C <> Неопределено Тогда
		DevID = Неопределено;
		ZR1C.Подключить(DevID);
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьЗначениеКомПорт()
	ЗначениеНастройки = ХранилищеОбщихНастроек.Загрузить("ТерминалВыдачиПЛ", "ПортКОДОС");
