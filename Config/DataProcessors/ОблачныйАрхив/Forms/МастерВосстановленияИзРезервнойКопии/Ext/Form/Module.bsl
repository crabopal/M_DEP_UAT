...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Загрузка файла резервной копии';en='File download backup'"));
...
"DBName: %3"
"UIDName: %4"
"Размер файла: %5';en='Attempts to load a file with zero size. Download cancelled."
"Login: %1"
"UIDFile: %2"
"DBName: %3"
"UIDName: %4"
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName)';en='Configuring the backup file upload, creating a directory structure (UIDName)'"));
...
"%1"
"по причине:"
"%2';en='Failed to create the structure of the working directory:"
"%1:"
"for a reason:"
"%2'"),
				лкКаталогВременныхФайлов,
...
"%1"
"Описание результата:"
"%2';en='Creation of the directory"
"%1"
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile)';en='Configuration the backup file, creating the directory structure (UIDName\UIDFile)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_BackUp
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\BackUp)';en='Configuration backup file, creating the directory structure (UIDName\UIDFile\BackUp)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_Downloads
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Downloads)';en='Configuration backup file, creating the directory structure (UIDName\UIDFile\Downloads)'"));
...
#Область НастройкаСтраницы_ЗагрузкаФайла_СозданиеСтруктурыКаталогов_UIDName_UIDFile_Scripts
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru='Настройка загрузки файла резервной копии, создание структуры каталогов (UIDName\UIDFile\Scripts)';en='Configuration backup file, creating the directory structure (UIDName\UIDFile\Scripts)'"));
...
#Область ПолучениеКлючаАвторизации
	Если ПустаяСтрока(ЭтотОбъект.authKey) Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru='Получение ключа авторизации (authKey) из приватных веб-сервисов';en='Getting authorization key (authKey) from private web services'"));
...
"Код результата: %1"
"Описание результата:"
"%2';en='Completed receiving authorization key."
"Result code: %1"
...
"DBName: %3"
"UIDName: %4"
"Размер файла: %5';en='Initialization completed (ПриСозданииНаСервере):"
"Login: %1"
"UIDFile: %2"
"DBName: %3"
"UIDName: %4"
...
"Вначале отмените загрузку.';en='It is not allowed to close the form while loading the file."
"First, cancel the download.'"), // Пояснение.
...
Процедура ПриОткрытии(Отказ)
	...
	#Если НЕ ВебКлиент Тогда
		...
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загрузка резервной копии базы %1';en='Download of database backup %1'"),
			ЭтотОбъект.DBName);

		// Текущее состояние загрузки.
		ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Проверка загруженного файла...';en='Checking downloaded file...'");
		...
		Иначе
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Загрузка файла...';en='Download file ...'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			Элементы.ДекорацияОшибкаТекст.Подсказка = "";
			Элементы.ГруппаСтраницыСостояниеЗагрузки.ТекущаяСтраница = Элементы.ГруппаСостояние;
...
Процедура ПутьДругогоКаталогаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		ДиалогВыбора.Заголовок = НСтр("ru='Выбор каталога восстановления резервной копии';en='Select directory restore from backup'");
...
Процедура КомандаВосстановить(Команда)
	...
	Если НЕ ПустаяСтрока(Результат) Тогда
		...
		Если Каталог.Существует() Тогда
			...
			Иначе
				ТекстСообщения = НСтр("ru='Необходимо указать имя каталога, а не имя файла.';en='It is necessary to specify a directory name, not the file name.'");
		...
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Каталога %1 не существует. проверьте правильность написания имени каталога.';en='Directory %1 does not exist. check the spelling of the directory name.'"),
				Результат);
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Поле  = "ПутьДругогоКаталога";
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru='Необходимо выбрать каталог восстановления резервной копии.';en='It is necessary to select directory to restore backup.'");
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Поле  = "ПутьДругогоКаталога";
		Сообщение.УстановитьДанные(Объект);
...
Процедура КомандаОтменаЗагрузки(Команда)
	...
	Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Отменить загрузку?';en='Cancel download?'"), // ТекстВопроса
			РежимДиалогаВопрос.ДаНет, // Кнопки
			10, // Таймаут
			КодВозвратаДиалога.Нет, // КнопкаПоУмолчанию
			НСтр("ru='Подтверждение';en='Confirmation'"), // Заголовок
...
Процедура КомандаДалее(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаВыборКаталога Тогда
		...
		Если ПустаяСтрока(ЭтотОбъект.ПутьДругогоКаталога) Тогда
			ТекстСообщения = НСтр("ru='Необходимо выбрать каталог для восстановления.';en='It is necessary to select directory for restore.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.Поле  = "ПутьДругогоКаталога";
			Сообщение.УстановитьДанные(Объект);
		...
		Иначе
			...
			Если ТекущийКаталогИБ = КаталогВосстановления Тогда
				Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
					НСтр("ru='Нельзя восстанавливать резервную копию в каталог с текущей информационной базой.';en='It is not allowed to restore a backup to a directory with the current information base.'");
			...
			Иначе
				...
				Иначе
					Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Каталог %1 не существует.';en='Directory %1 does not exist.'"),
...
#Область СтраницаЗагрузкаФайла
		...
		лкДалее.Заголовок       = НСтр("ru='Загрузить';en='Import'");
		...
		ИначеЕсли Форма.ИдентификаторСостоянияЗагрузкиФайла = "ФайлЗагружен" Тогда
			...
			лкДалее.Заголовок   = НСтр("ru='Распаковать';en='Unpack'");
		...
		ИначеЕсли Форма.ИдентификаторСостоянияЗагрузкиФайла = "ОшибкиЗагрузки" Тогда
			лкДалее.Заголовок   = НСтр("ru='Загрузить заново';en='Download again'");
			лкДалее.Доступность    = Истина;
			лкЗакрыть.Доступность  = Истина;
...
#Область СтраницаВыборКаталога
		...
		Форма.Заголовок = НСтр("ru='Выберите каталог для разархивации';en='Select a directory for unzipping'");

		лкОтмена.Видимость    = Ложь;
		лкНазад.Видимость     = Ложь;
		лкДалее.Видимость     = Истина;
		лкДалее.Доступность   = Истина;
		лкДалее.Заголовок     = НСтр("ru='Разархивировать';en='Unzip'");
		лкЗакрыть.Видимость   = Истина;
		лкЗакрыть.Доступность = Истина;
...
#Область СтраницаОтключениеСеансов
		...
		Форма.Заголовок = НСтр("ru='Отключение активных сеансов';en='Disconnecting active sessions'");
...
#Область СтраницаУспешноеЗавершение
		...
		Форма.Заголовок = НСтр("ru='Данные успешно восстановлены из резервной копии';en='Data successfully restored from backup'");
		...
		лкЗакрыть.Заголовок = НСтр("ru='Готово';en='Done'");
	...
	#Если НЕ ВебКлиент Тогда
		...
		Попытка
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru='Соединение с сервером для загрузки файла резервной копии';en='Connection with the server for download the backup file'"));
			...
			ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
				ЭтотОбъект.КонтекстВыполнения,
				0,
				НСтр("ru='Успешное подключение к серверу для загрузки файла резервной копии';en='A successful connection to the server for the backup file download'"),
				Неопределено);
...
"%1'"),
		...
		Иначе
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Ошибка подключения к http-сервисам.';en='Failed to connect to http services.'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
			Элементы.ДекорацияОшибкаТекст.Подсказка =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка подключения к http-сервисам:"
"%1';en='Failed to connect to http-services:"
"%1'"),
					ПодробноеПредставлениеОшибки);
			Элементы.ГруппаСтраницыСостояниеЗагрузки.ТекущаяСтраница = Элементы.ГруппаОшибка;
		КонецЕсли;
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru='Загрузка части файла резервной копии';en='Download of the part of the backup file'"));
			КодРезультата = 0;
			ОписаниеРезультата = "";
...
"%1';en='An error occurred downloading part of the file:"
"%1'")
								+ Символы.ПС,
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				КонецПопытки;
...
"%2'")
				...
				Иначе
					...
					ИначеЕсли КодВозврата = "7" Тогда // Переполучить ключ авторизации
						...
						ОписаниеРезультата = ОписаниеРезультата
							+ НСтр("ru='Процесс загрузки был слишком длительный, ключ авторизации просрочился и будет получен заново.';en='Download process was too long, the authorization key prorocols and will be received again.'")
								+ Символы.ПС;
					Иначе
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Код возврата (errorCode) вернул ошибку: %1';en='Return code (errorCode) returned error: %1'")
									+ Символы.ПС,
								КодВозврата);
					КонецЕсли;
...
"Размер файла (байт): %2"
"Контрольная сумма (md5): %3';en='Successfully downloaded part of the file."
"The name of the temporary file: %1"
"File size (bytes): %2"
"Checksum (md5): %3'")
											+ Символы.ПС,
							...
							Иначе
								...
								ОписаниеРезультата = ОписаниеРезультата
									+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Не совпадает ожидаемая (%1) и реальная (%2) контрольная сумма (md5) части файла.';en='Expected (%1) and real (%2) checksum (md5) of the file does not match .'")
											+ Символы.ПС,
										ТекущаяСтрока.КонтрольнаяСумма,
						...
						Иначе
							...
							ОписаниеРезультата = ОписаниеРезультата
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Не совпадает ожидаемый (%1) и реальный (%2) размер части файла.';en='Expected (%1) and real (%2) size of the file part does not match.'")
										+ Символы.ПС,
									ТекущаяСтрока.Размер,
					...
					Иначе
						...
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Не найден временный файл (%1). Возможно, он не был загружен.';en='Not found the temporary file (%1). Perhaps it was not loaded.'")
									+ Символы.ПС,
								ТекущаяСтрока.ИмяВременногоФайла);
					КонецЕсли;
				КонецЕсли;
			...
			Если ЭтотОбъект.КоличествоСбойныхЧастей = 0 Тогда
				ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Загружено %1 частей из %2 (%3 из %4).';en='%1 parts of %2 downloaded (%3 of %4).'"),
...
"Во время загрузки были сбои: %5';en='Downloaded %1 of the parts of %2 (%3 of %4)."
"While loading failed: %5'"),
					ЭтотОбъект.ЧастиФайла.Индекс(ТекущаяСтрока) + 1,
					ЭтотОбъект.ЧастиФайла.Количество(),
					ОблачныйАрхивКлиентСервер.ПолучитьПредставлениеРазмера(ЭтотОбъект.ВсегоЗагруженоБайт),
					ОблачныйАрхивКлиентСервер.ПолучитьПредставлениеРазмера(ЭтотОбъект.filesize),
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru='Завершение загрузки файла резервной копии';en='Completing the download of the backup file'"));
			КодРезультата = 0;
			ОписаниеРезультата = "";
			...
			Если МассивНеЗагруженных.Количество() = 0 Тогда
				...
				ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Объединение частей файла в один архив...';en='Uniting the parts of the file in one archive...'");
				ЭтотОбъект.ОбновитьОтображениеДанных();
				...
				ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
					ЭтотОбъект.КонтекстВыполнения,
					КодРезультата,
					НСтр("ru='Загрузка файла резервной копии успешно завершена';en='Backup file download is completed successfully'"),
					Неопределено);
			...
			Иначе // Есть незагруженные части файлов
				...
				Если ЕстьЧтоЗагружать = Ложь Тогда
					...
					ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ОшибкиЗагрузки"; // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
					// Текущее состояние загрузки.
					ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Остались незагруженные части файла.';en='Unloaded parts of the file are remained.'");
					Элементы.ДекорацияОшибкаТекст.Подсказка =
						НСтр("ru='Остались незагруженные части файла,"
"по которым не осталось попыток загрузки."
"Загрузка завершена с ошибками.';en='There were unloaded parts of the file,"
"for which there are no attempts to download."
"Download completed with errors.'");
					...
					ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
						ЭтотОбъект.КонтекстВыполнения,
						1002,
						НСтр("ru='Загрузка файла резервной копии не завершена из-за ошибок';en='Backup file download was not completed due to errors'"),
						Неопределено);
	...
	#Если НЕ ВебКлиент Тогда
		...
		Элементы.ДекорацияУспешноеЗавершениеТекст.Заголовок =
			Новый ФорматированнаяСтрока(
				НСтр("ru='Информационная база из резервной копии успешно восстановлена в каталог';en='Information database from a backup restored successfully in the directory'"),
				Символы.ПС,
				Новый ФорматированнаяСтрока(
					ЭтотОбъект.ПутьДругогоКаталога,
					,
		...
		// Текущее состояние загрузки.
		ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Очистка каталога перед распаковкой файла...';en='Cleaning the directory before unpacking the file...'");
		Элементы.ДекорацияВыборКаталогаОшибкаТекст.Заголовок = "";
		Элементы.ГруппаВыборКаталогаПодвал.ТекущаяСтраница = Элементы.ГруппаВыборКаталогаПодвалПредупреждения;
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru='Очистка каталога-получателя перед разархивацией загруженного файла резервной копии';en='Clearing the directory of the recipient before unzipping of the downloaded backup file'"));
		...
		Если ЕстьОшибкиУдаления = Истина Тогда
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Ошибка разархивации файла.';en='Error unzipping the file.'");
			ЭтотОбъект.ОбновитьОтображениеДанных();
...
"Возможно, запущена информационная база, связанная с этим каталогом.';en='Error cleaning directory."
"Perhaps, the information base associated with this directory has been launched.'");
			Элементы.ГруппаВыборКаталогаПодвал.ТекущаяСтраница = Элементы.ГруппаВыборКаталогаПодвалОшибка;
			УправлениеФормой(ЭтотОбъект);
...
"Путь для очистки: %3"
"Ошибка:"
"%4';en='Error cleaning receiver directory before unpacking the backup."
"Path to the archiver: %1"
"Path to the uploaded file: %2"
"Path to clean: %3"
"Error:"
"%4'")
						+ Символы.ПС,
...
"Путь к архиватору: %1"
"Путь к загруженному файлу: %2"
"Путь для очистки: %3';en='Successful cleaning the receiver directory ."
"Path to the archiver: %1"
"Path to the uploaded file: %2"
"Path to clean: %3'")
						+ Символы.ПС,
			...
			// Текущее состояние загрузки.
			ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Распаковка файла...';en='Unpacking file...'");
			...
			Попытка
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					НСтр("ru='Разархивация загруженного файла резервной копии';en='Unzipping the downloaded backup file'"));
...
"Командная строка: %5"
"Путь к командному файлу: %6';en='A successful unzipping backup file."
"Path tothe archiver: %1 "
"Path to the downloaded file: %2"
"Size of the uploaded file: %3"
"Applications return code: %4"
"Command string: %5"
"Path to the command file: %6'")
							+ Символы.ПС,
						ПутьКАрхиватору,
						ПутьКЗагруженномуФайлу,
...
"Ошибка:"
"%7';en='Error unzipping backup file."
"Path to the archiver: %1"
"Path to the downloaded file: %2"
"Size of the uploaded file: %3"
"Applications return code: %4"
...
Процедура УдалитьВременныеФайлы()
	...
	ЭтотОбъект.ПредставлениеТекущегоСостоянияЗагрузкиФайла = НСтр("ru='Удаление временных файлов...';en='Delete temporary files...'");
	ЭтотОбъект.ОбновитьОтображениеДанных();
	УдалитьФайлы(
		ЭтотОбъект.КаталогРабочий
			+ ПолучитьРазделительПутиКлиента()
...
"%4';en='Download backup file, final report."
"Start time (ms): %1"
"End time (ms): %2"
"Duration (ms): %3"
...
"%4'")
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Веб-сервисы';en='LIS:Cloud-basedarchive.Web-servises'"), // Имя события
		НСтр("ru='Облачный архив. Веб-сервисы. Загрузка файла резервной копии';en='Cloud archive. Web-services. Backup file download'"), // ИмяСобытия
...
Процедура ПослеВопросаПодтвержденияОтменыЗагрузки(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru='Отмена загрузки файла резервной копии';en='Backup file download cancel'"));
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			ЭтотОбъект.КонтекстВыполнения,
			0,
			НСтр("ru='Отменена загрузка файла резервной копии';en='The download of the backup file is cancelled'"),
			Неопределено);
