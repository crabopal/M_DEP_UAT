...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияГиперссылкаКабинетКлиента.Подсказка =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Логин: %1';en='Login: %1'"),
			?(ПустаяСтрока(ЭтотОбъект.Логин), НСтр("ru='не введен, будет запрошен в браузере.';en='not entered, will be requested in the browser.'"), ЭтотОбъект.Логин));
...
Процедура КомандаУдалитьФайл(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		СписокКнопок.Добавить(НСтр("ru='Удалить';en='Delete'")); // Идентификатор
		СписокКнопок.Добавить(НСтр("ru='Отмена';en='Cancel'")); // Идентификатор
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Удалить файл с сервера?';en='Delete file from server?'"),
			СписокКнопок,
			0,
			НСтр("ru='Отмена';en='Cancel'"), // Идентификатор
			НСтр("ru='Запрос на удаление файла с сервера';en='Request to delete file from server'"));
	КонецЕсли;
...
Процедура УдалитьФайл_НаКлиенте(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = НСтр("ru='Удалить';en='Delete'") Тогда // Идентификатор
