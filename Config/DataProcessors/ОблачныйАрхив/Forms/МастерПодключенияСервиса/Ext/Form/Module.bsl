...
#Область ПолучениеПараметров
	...
	Элементы.ДекорацияВерсияИРазмерАгентаТекст.Заголовок =
		Новый ФорматированнаяСтрока(
			НСтр("ru='Версия дистрибутива:';en='Distribution version:'"),
			" ",
			Новый ФорматированнаяСтрока(
			...
			Символы.ПС,
			НСтр("ru='Размер дистрибутива:';en='Distribution size:'"),
			" ",
			Новый ФорматированнаяСтрока(
				Формат(ЭтотОбъект.РазмерДистрибутиваБайт, "ЧЦ=10; ЧДЦ=2; ЧС=6; ЧРД=,; ЧРГ=' '; ЧН=0,00; ЧГ=3,0") + " " + НСтр("ru='МБайт';en='MB'"),
				Новый Шрифт(Элементы.ДекорацияВерсияИРазмерАгентаТекст.Шрифт,,,Истина))); // Жирный.
			...
			Если (ЭтотОбъект.АгентКопированияУстановлен) И (ЭтотОбъект.ТребуетсяОбновление = 0) Тогда
				Если (ЭтотОбъект.АгентКопированияАктивированНаТекущийЛогин = Истина) Тогда
					Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаУспешноТекст.Заголовок =
						Новый ФорматированнаяСтрока(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Установлена версия Агента резервного копирования: %1';en='Installed the Agent version backup: %1'"),
							...
							Символы.ПС,
							Символы.ПС,
							НСтр("ru='Для настройки расписания автоматического резервного копирования необходимо запустить 1С:Предприятие на компьютере, на котором установлен Агент резервного копирования.';en='For settings the automatic backup schedule it is required to run 1C:Enterprise on the computer on which the Agent backup set.'"),
							Символы.ПС,
							Символы.ПС,
							НСтр("ru='Просматривать список созданных резервных копий можно с любого компьютера.';en='You can view the list of created backups from any computer.'"));
				...
				Иначе
					...
					Иначе
						Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаСОшибкамиТекст.Заголовок =
							Новый ФорматированнаяСтрока(
							НСтр("ru='Недостаточно свободных лицензий для активации Агента резервного копирования.';en='Not enough free licenses to activate the Backup Agent.'"),
							Символы.ПС,
							Символы.ПС,
							НСтр("ru='Купить дополнительные лицензии на использование Агента резервного копирования на этом компьютере можно в';en='To purchase additional licenses to use the backup Agent on this computer it is allowed in'"),
							" ",
							Новый ФорматированнаяСтрока(
								НСтр("ru='личном кабинете';en='personal account'"),
								,
...
"Для этого из';en='Also, in your account it is allowed to free up licenses from other computers."
"To do this'"),
							" ",
							Новый ФорматированнаяСтрока(
								НСтр("ru='личного кабинета';en='personal account'"),
								,
								ЭтотОбъект.ЦветГиперссылки,
								,
								"backup1C:OpenWebPersonalAccount_DeactivateAgent"),
							" ",
							НСтр("ru='необходимо выбрать неиспользуемого Агента резервного копирования и нажать кнопку Освободить.';en='necessary to choose unused backup Agent and press Release button.'"),
							Символы.ПС);
						ПереключитьсяНаСтраницуНаСервере("СтраницаУстановкаАгентаРезервногоКопированияЗавершенаСОшибками");
					КонецЕсли;
				КонецЕсли;
...
Процедура КаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если Не ВебКлиент Тогда
		...
		Если Элемент.Имя = "КаталогЛогов" Тогда
			ДиалогВыбора.Заголовок = НСтр("ru='Выбор каталога для сохранения лог-файлов';en='Select directory to save log files'");
			ДиалогВыбора.Каталог = ЭтотОбъект.КаталогЛогов;
		ИначеЕсли Элемент.Имя = "КаталогСтатистики" Тогда
			ДиалогВыбора.Заголовок = НСтр("ru='Выбор каталога для сохранения файлов статистики';en='Choice of directory for saving statistics files'");
			ДиалогВыбора.Каталог = ЭтотОбъект.КаталогСтатистики;
		ИначеЕсли Элемент.Имя = "КаталогРабочий" Тогда
			ДиалогВыбора.Заголовок = НСтр("ru='Выбор каталога для сохранения рабочих и временных файлов';en='Choice of directory to store working and temporary files'");
...
Процедура ЗагрузитьСтатистику(СрокЖизниСтатистики=300, СписокШагов = "")
	...
	МассивШагов = Новый Массив;
		...
		ШагСбораДанных = ОблачныйАрхивКлиентСервер.ПолучитьОписаниеШагаСбораДанных();
			...
			ШагСбораДанных.ОписаниеШага          = НСтр("ru='Сбор информации об актуальных версиях Агентов резервного копирования';en='Gathering information about the current versions of backup Agents'");
			ШагСбораДанных.СрокУстареванияСекунд = СрокЖизниСтатистики; // Обновлять только если данные были собраны > 5 минут назад.
		Если (ПустаяСтрока(СписокШагов))
				ИЛИ (НЕ ПустаяСтрока(СписокШагов) И СтрНайти(СписокШагов, ШагСбораДанных.КодШага)) Тогда
			МассивШагов.Добавить(ШагСбораДанных);
...
#Область СтраницаПодключениеКВебИТС
		...
		лкДалее.Заголовок = НСтр("ru='Подключиться';en='Connect'");
...
#Область СтраницаУстановкаАгентаРезервногоКопирования
		...
		лкДалее.Заголовок = НСтр("ru='Установить';en='Set'");
		лкДалее.КнопкаПоУмолчанию = Истина;
...
#Область СтраницаУстановкаАгентаРезервногоКопированияЗавершенаУспешно
		...
		лкДалее.Заголовок = НСтр("ru='Настроить';en='Customize'");
		лкДалее.КнопкаПоУмолчанию = Истина;
...
"Проверка правильности логина и пароля.';en='Connection to services of online support."
"Verifying a username and password.'");
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаУстановкаАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ru='Проверка установки Агента резервного копирования.';en='Verifying the installation of the backup Agent.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
"Проверьте правильность логина и пароля и повторите попытку.';en='Уrror occurred while connecting to web services."
...
Процедура ВыполнитьПереходНаКлиенте(ИмяТекущейСтраницы, НаправлениеПерехода)
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаПодключениеКВебИТС" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ru='Подключение к сервисам интернет-поддержки.';en='Connection to services of online support.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
"To do this'"),
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаУстановкаАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			// Всегда переключаться на страницу длительных операций.
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("ru='Загрузка и установка Агента резервного копирования.';en='Download and install of backup agent.'");
			ПереключитьсяНаСтраницуНаКлиенте("СтраницаДлительнаяОперация");
...
#Область ЗагрузкаФайлаИнсталляции
		...
		ОбновитьНадписиНаСервере(НСтр("ru='Загрузка файла установки агента резервного копирования.';en='Download the backup agent installation file.'"));
			...
			Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
				...
				Если СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
					...
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ru='Агент резервного копирования успешно загружен';en='Backup agent successfully loaded'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если НеобходимаЗагрузка = Истина Тогда
				...
				Если ФайлУстановкиСуществует = Истина Тогда
					...
					Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
						...
						Если НЕ СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
							...
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
								НСтр("ru='Файл дистрибутива повреждён (контрольная сумма загруженного дистрибутива отличается от указанной на сайте)';en='Installation file is corrupted (checksum of the downloaded distribution is different from that indicated on the website)'"));
						Иначе
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
					...
					Иначе
						...
						ОписаниеРезультата = Новый ФорматированнаяСтрока(
							ОписаниеРезультата,
							Символы.ПС,
							НСтр("ru='Файл дистрибутива повреждён (размер загруженного дистрибутива отличается от указанного на сайте)';en='Installation file is corrupted (the size of the downloaded distribution differs from that indicated on the website)'"));
					КонецЕсли;
				Иначе
					ЕстьОшибки = Истина;
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ru='Файл дистрибутива отсутствует во временном каталоге.';en='Distribution file is missing in the temporary directory.'"));
				КонецЕсли;
			Иначе // Не удалось загрузить...
				...
				ОписаниеРезультата = Новый ФорматированнаяСтрока(
					ОписаниеРезультата,
					Символы.ПС,
					НСтр("ru='Файл дистрибутива не загружен. Произошли непредвиденные ошибки.';en='Distribution file is not loaded. Encountered an unexpected error.'"));
			КонецЕсли;
		КонецЕсли;
...
#Область ЗапускИнсталляции
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Запуск установки агента резервного копирования.';en='Starting of installation of backup agent.'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Агент резервного копирования успешно установлен';en='Backup agent is installed successfully'"));
...
#Область СборИнформации_ИнформацияОКлиенте
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Сбор данных для агента резервного копирования (анализ состояния компьютера).';en='Data collection for backup agent (analyzing the state of the computer).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Все операции с агентом на компьютере завершены.';en='All operations with the agent on a computer has been completed.'"));
...
#Область СозданиеФайловНастроек
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Создание настроек по-умолчанию для агента резервного копирования.';en='Create default settings for the backup agent.'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Успешно сохранены настройки для агента резервного копирования';en='Successfully saved settings for backup agent'"));
...
#Область АктивацияАгентаКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Активация агента резервного копирования (получение ключей, первый запуск).';en='Activation of backup agent (getting the keys, first start).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Активация Агента резервного копирования: успешно получены коды запуска, завершён первый запуск';en='Backup Agent activation performed successfully: retrieved the start codes, completed the first start'"));
			...
			Если (НЕ ПустаяСтрока(РезультатАктивацииАгента.ЛогинАктивации))
				...
				Исключение
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ru='Не удалось активировать аккаунт.';en='Failed to activate account.'"));
				КонецПопытки;
				...
				Если КодВозврата = Неопределено Тогда
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
						ОписаниеРезультата,
						Символы.ПС,
						НСтр("ru='Не удалось запустить Агент резервного копирования для активации аккаунта.';en='Unable to start backup Agent to activate the account.'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
#Область СборИнформации_ИнформацияОКлиенте_АктивацииАгентовКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("ru='Сбор данных для агента резервного копирования (анализ состояния компьютера, состояние активации).';en='Data collection for backup agent (analyzing the state of the computer, the activation state).'"));
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Проверка состояния активации завершена.';en='Check the status of activation complete.'"));
...
#Область Окончание
		...
		ОбновитьНадписиНаСервере(НСтр("ru='Всё готово';en='Everything is ready'"));
