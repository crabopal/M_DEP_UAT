...
Функция ОкруглитьЧислоПоФормату(ИсходноеЧисло)
	...
	Значение = Окр(ИсходноеЧисло / Делитель, ТочностьЕдиницыИзмерения);
	ФорматПоказателя = "ЧЦ = 15; ЧДЦ = " + Строка(ТочностьЕдиницыИзмерения);
	Возврат  Формат(Значение,ФорматПоказателя);
			...
			Для Каждого СтрокаПоказатели Из Показатели Цикл
				...
				Для Индекс=0 По ТаблицаСценарии.Количество()-1 Цикл
					...
					Если Номер = Индекс Тогда
						...
						ТекстЗапроса = ТекстЗапроса + ",
						|	СУММА(ЕСТЬNULL("+СтрокаПоказатели.Имя+"Оборот,0)) КАК " + СтрокаПоказатели.Имя+"_"+Индекс;
...
Процедура СформироватьНастройкиОтчета(МассивОтбора,СтруктураПредставлениеПолей)
	...
	|ДанныеСценариев.СтатьяОборотов.Код     //ЮТ
	|{УПОРЯДОЧИТЬ ПО ДанныеСценариев.НомерСценария,
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	мНазваниеОтчета = "Сравнительный анализ оборотов";
	мНаДату = Ложь;
	...
	Если ТаблицаСценарии.Количество()>0 Тогда
		...
		Бюджетирование.ЗаполнитьПредставленияПолей(СтруктураПредставлениеПолей, ПостроительОтчета);
		Бюджетирование.ОчиститьДополнительныеПоляПостроителя(ПостроительОтчета);
		Бюджетирование.ЗаполнитьОтбор(МассивОтбора, ПостроительОтчета);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если (Выборка.Группировка()<>"НомерСценария") и (Выборка.Группировка()<>"Период")  Тогда  // Не выводим группировку, разворачивающуюся по горизонтали.
			...
			Если ТипЗаписиВыборки = ТипЗаписиЗапроса.ИтогПоИерархии Тогда
				...
				Если ВыводитьКодСтатьи и НЕ ВыводитьКодСтатьиОтдельнойКолонкой Тогда //ЮТ
					Если ТипЗнч(Выборка[ИмяГруппировки]) = Тип("СправочникССылка.СтатьиОборотовПоБюджетам") Тогда    
						ЗначениеТекущейГруппировки = ЗначениеТекущейГруппировки + "," + СокрЛП(Выборка[ИмяГруппировки].Код);
					КонецЕсли;	
				КонецЕсли;	
			...
			ИначеЕсли ТипЗаписиВыборки = ТипЗаписиЗапроса.ИтогПоГруппировке Тогда
				...
				Если СтруктураВыводГруппировок.Свойство(ИмяГруппировки, МассивВыводГруппировок) Тогда
					...
					Для Каждого ВыводимаяГруппировка Из МассивВыводГруппировок Цикл
						...
						Если Найти(ВыводимаяГруппировка,"Период")>0 Тогда
							ЗначениеТекущейГруппировки=Бюджетирование.ПолучитьПериодСтрокой(Выборка[ВыводимаяГруппировка],СтрЗаменить(ВыводимаяГруппировка,"Период",""));
						Иначе
				...
				Если Найти(ИмяГруппировки,"Период")>0 Тогда
					ЗначениеТекущейГруппировки=Бюджетирование.ПолучитьПериодСтрокой(Выборка[ИмяГруппировки],СтрЗаменить(ИмяГруппировки,"Период",""));
				Иначе
					ЗначениеТекущейГруппировки = "" + Выборка[ИмяГруппировки];
					
					Если ВыводитьКодСтатьи И НЕ ВыводитьКодСтатьиОтдельнойКолонкой Тогда //ЮТ
						Если ТипЗнч(Выборка[ИмяГруппировки]) = Тип("СправочникССылка.СтатьиОборотовПоБюджетам") Тогда    //ЮТ
							ЗначениеТекущейГруппировки = ЗначениеТекущейГруппировки + "," + СокрЛП(Выборка[ИмяГруппировки].Код);
						КонецЕсли;	
					КонецЕсли;	
			...
			Если ВыводитьКодСтатьи И ВыводитьКодСтатьиОтдельнойКолонкой Тогда
				ОбластьОбщийОтступ.Параметры.Код = Выборка.СтатьяОборотов.Код;
			КонецЕсли;
			...
			//Область = ТабДок.Область(НачСтрока, 2, ТабДок.ВысотаТаблицы, ТабДок.ШиринаТаблицы); //ЮЦАУ
			...
			Если Выборка.СтатьяОборотов.ЭтоГруппа Тогда //ЮЦАУ
				мНастроек = Выборка.СтатьяОборотов.ХранилищеНастроекШрифта.Получить();
				Если  мНастроек <> Неопределено Тогда
					Если мНастроек.Количество() > 0 Тогда
						начКолонка = ? (ВыводитьКодСтатьиОтдельнойКолонкой,1,2);
						ТабДок.Область(ТабДок.ВысотаТаблицы, начКолонка, ТабДок.ВысотаТаблицы,2).Шрифт = мНастроек[0];
                        ТабДок.Область(ТабДок.ВысотаТаблицы, начКолонка, ТабДок.ВысотаТаблицы,ТабДок.ШиринаТаблицы).ЦветФона = мНастроек[1];
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь) Экспорт
	...
	СтруктураПараметров = Новый Структура;
	Если ВыводитьКодСтатьи И ВыводитьКодСтатьиОтдельнойКолонкой Тогда
		СтруктураПараметров.Вставить("ОбщийОтступ", Макет.ПолучитьОбласть("Код|Строка"));
	Иначе
	...
	Если Бюджетирование.ПроверитьПовторыИзмеренийПостроителя(ПостроительОтчета) = Ложь Тогда
		Возврат;
	КонецЕсли;
	
	Результат = ПостроительОтчета.Результат;
	Если ВыводитьКодСтатьи И ВыводитьКодСтатьиОтдельнойКолонкой Тогда
		ДокументРезультат.Вывести(Макет.ПолучитьОбласть("Код|ШапкаГруппы"));
	Иначе
	...
	Если КоличествоПоказателей>1 Тогда
		...
		Если ВыводитьКодСтатьи И ВыводитьКодСтатьиОтдельнойКолонкой Тогда
			ДокументРезультат.Вывести(Макет.ПолучитьОбласть("Код|ШапкаПоказатели"));
		Иначе
	...
	Если ВыводитьКодСтатьи И ВыводитьКодСтатьиОтдельнойКолонкой Тогда
		ДокументРезультат.Вывести(Макет.ПолучитьОбласть("Код|ОбщиеИтоги"));
	Иначе
	...
	Если ВыводитьКодСтатьи И ВыводитьКодСтатьиОтдельнойКолонкой Тогда
		ДокументРезультат.Вывести(Макет.ПолучитьОбласть("Код|Подвал"));
	Иначе
	...
	// СогласовательнаяКомиссия
	
	
	ОбластьЧлена   = Макет.ПолучитьОбласть("Члены");
	
	Организация = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(глЗначениеПеременной("глТекущийПользователь"), "ОсновнаяОрганизация");
	
	ТаблицаСогласовательнаяКомиссия = СогласовательнаяКомиссия.Выгрузить();
	КоммиссияИсполнитель = ТаблицаСогласовательнаяКомиссия.Найти(Истина, "Исполнитель");

	Для Каждого Строка Из ТаблицаСогласовательнаяКомиссия Цикл
		
		Если Строка.Исполнитель Тогда
			Продолжить;
		КонецЕсли;
		ДанныеЧлена = ОбщегоНазначения.ДанныеФизЛица(Организация, Строка.ФизЛицо, ОбщегоНазначения.ПолучитьРабочуюДату());
		Инд = Инд + 1;
		ОбластьЧлена.Параметры["ДолжностьЧлена"] = ДанныеЧлена.Должность;
		ОбластьЧлена.Параметры["ФИОЧлена"]      = ДанныеЧлена.Представление;
		
		ДокументРезультат.Вывести(ОбластьЧлена);
		
	КонецЦикла;
	
	ОбластьИсполнитель   = Макет.ПолучитьОбласть("Исполнитель");
	
	Если НЕ (КоммиссияИсполнитель = Неопределено) Тогда
		ДанныеИсполнителя = ОбщегоНазначения.ДанныеФизЛица(Организация, КоммиссияИсполнитель.ФизЛицо,ОбщегоНазначения.ПолучитьРабочуюДату());
		ОбластьИсполнитель.Параметры.ДолжностьИсполнителя    = ДанныеИсполнителя.Должность;
		ОбластьИсполнитель.Параметры.ФИОИсполнителя          = ДанныеИсполнителя.Представление;
	Иначе
		ОбластьИсполнитель.Параметры.ДолжностьИсполнителя    = "";
		ОбластьИсполнитель.Параметры.ФИОИсполнителя          = "";
	КонецЕсли;
	
	ДокументРезультат.Вывести(ОбластьИсполнитель);
	//СогласовательнаяКомиссия
...
Процедура ВывестиСтрокуШапки(ВыборкаГоризонтальная, СтруктураПараметров, ЗначениеВыборкиДанные,СтрокаДанныхОтчета)
	...
	ЗначениеГруппировкиПериоды=Бюджетирование.ПолучитьПериодСтрокой(ВыборкаГоризонтальная["Период"],ПериодДанных);
	...
	Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Группировка()="Период" Тогда
				...
				Если мЗначимыеГруппировкиГор.НайтиПоЗначению(СтрТекущийПуть)<>Неопределено Тогда
					...
					Для Каждого Имя Из СтруктураПараметров.МассивПоказатели Цикл
						...
						ТекущееЗначение = Выборка[Имя+"_"+мСоответствиеНомера[мВыведеноПериодов]];
						ТекущееЗначение = ?(ТипЗнч(ТекущееЗначение) = Тип("NULL"),0,ТекущееЗначение); 
						Если ПоложительныеРасходы Тогда 
							ТекущееЗначение = ОкруглитьЧислоПоФормату(?(ТекущееЗначение<0,ТекущееЗначение*(-1),ТекущееЗначение));
						Иначе	
							ТекущееЗначение = ОкруглитьЧислоПоФормату(ТекущееЗначение);
						КонецЕсли;
		...
		Если Выборка.Группировка()="НомерСценария" Тогда
			...
			Для Каждого Имя Из СтруктураПараметров.МассивПоказатели Цикл
				...
				//Если ТипЗнч(Выборка.СтатьяОборотов) = Тип("СправочникСсылка.СтатьиОборотовПоБюджетам") Тогда
				//	Если Выборка.СтатьяОборотов.ЭтоГруппа Тогда
				//		ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя = Формат(ТекущееЗначение, ФорматПоказателя);
				//	Иначе
				Если ПоложительныеРасходы Тогда 
					ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя = ОкруглитьЧислоПоФормату(?(ТекущееЗначение<0,ТекущееЗначение*(-1),ТекущееЗначение));
				Иначе	
					//ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя = Формат(ТекущееЗначение, ФорматПоказателя);
					ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя = ОкруглитьЧислоПоФормату(ТекущееЗначение);
				КонецЕсли;
				//	КонецЕсли;
				//Иначе
				//	ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя = Формат(ТекущееЗначение, ФорматПоказателя);
			...
			Если НЕ СтрокаДанныхОтчета.Эталон Тогда
				...
				Если ВыводитьАбсолютныеОтклонения Тогда
					...
					Для Каждого Имя Из СтруктураПараметров.МассивПоказатели Цикл
						...
						ОбластьЗначенияПоказателя.Параметры.ЗначениеПоказателя = ОкруглитьЧислоПоФормату(ТекущиеДанные-ЗначениеЭталон);
...
Процедура РасшифровкаТекущимОтчетом (Расшифровка,ЭтотОтчет,ЗначениеВыбора)
	...
	// Настройка отчета
		...
		Параметры.Вставить("ТочностьЕдиницыИзмерения",ТочностьЕдиницыИзмерения);
