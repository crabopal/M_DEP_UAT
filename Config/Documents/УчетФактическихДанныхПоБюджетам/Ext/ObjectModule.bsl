...
Процедура ПолучитьДанныеПоСтатье(Статья,СписокЦФО,ТабОперации,НомерРасчета)
	...
	//Если СокрЛП(Статья.ТекстЗапроса) = "" Тогда
	//	Сообщить("Нет данных об источнике данных "+Статья.СтатьяОборотовПредставление);
	//	Возврат;
	//КонецЕсли;
	...
	Если Статья.СпособОпределенияВалютыОборотаФакт=Перечисления.СпособыОпределенияВалютыОборотаПоСтатье.ВалютаУправленческогоУчета Тогда
		ТабОперации.ЗаполнитьЗначения(Константы.ВалютаРегламентированногоУчета.Получить(),"Валюта");
...
Функция СформироватьЗапросПоДаннымДокумента()
	...
	ТекстЗапроса="ВЫБРАТЬ
				...
				Запрос.УстановитьПараметр("ВалютаУпрУчета",Константы.ВалютаРегламентированногоУчета.Получить());
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	уатОбщегоНазначенияТиповые.уатПроверитьЗаполнениеТабличнойЧасти(ЭтотОбъект, "БюджетныеОперации", СтруктураОбязательныхПолейТЧ(),Отказ, Заголовок);	
